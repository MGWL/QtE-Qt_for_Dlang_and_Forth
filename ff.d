...
class ClassMain: QMainWindow {
    ...
    QTextEdit   te_list;                    // Вывод результата
    ...
    void knpOpenDir() {     // Открыть каталог с файлом
        try {
            QString tmpQstr = new QString();
            te_list.getQStringCursor(tmpQstr);
            string nameProc;
            nameProc = tmpQstr.fromUnicode(nameProc, UTF_8);
    ...
    void ViewStrs() {              // Искать вхождения строк
        ...
        void PrintEk(StNameFile el) {
            char[] fullName = mPath[el.FullPath] ~ dirSeparator ~ el.NameFile;
            qstr.toUnicode(cast(char*)(fullName ~ 0 ~ 0).ptr, WIN_1251);  te_list.append(qstr);
        }
